package com.edso.resume.lib.common;

public class TypeConfig {
    public static final String SELECT = "select";
    public static final String CREATE = "create";
    public static final String UPDATE = "update";
    public static final String DELETE = "delete";
    //Ứng viên
    public static final String CALENDAR_CANDIDATE = "calendar_candidate";
    //Người giới thiệu
    public static final String CALENDAR_PRESENTER = "calendar_presenter";
    //Hội đồng tuyển dụng
    public static final String CALENDAR_INTERVIEWER = "calendar_interviewer";
    //Người liên quan
    public static final String CALENDAR_RELATED_PEOPLE = "calendar_related_people";
    //Ứng viên
    public static final String CALENDARS_CANDIDATE = "calendars_candidate";
    //Người giới thiệu
    public static final String CALENDARS_PRESENTER = "calendars_presenter";
    //Hội đồng tuyển dụng
    public static final String CALENDARS_INTERVIEWER = "calendars_interviewer";
    //Người liên quan
    public static final String CALENDARS_RELATED_PEOPLE = "calendars_related_people";
    //Người giới thiệu
    public static final String ROUND_PRESENTER = "round_presenter";
    //Ứng viên
    public static final String ROUND_CANDIDATE = "round_candidate";
    //Người liên quan
    public static final String ROUND_RELATED_PEOPLE = "round_related_people";
    //Người liên quan
    public static final String ROUND_INTERVIEWER = "round_interviewer";
    //Ứng viên
    public static final String REJECT_CANDIDATE = "reject_candidate";
    //Người giới thiệu
    public static final String REJECT_PRESENTER = "reject_presenter";
    //Người liên quan
    public static final String REJECT_RELATED_PEOPLE = "reject_related_people";
    //Hội đồng tuyển dụng
    public static final String REJECT_INTERVIEWER = "reject_interviewer";
    //Ứng viên
    public static final String EMAILS_CANDIDATE = "emails_candidate";
}
